...
class SegmentedControls extends React.Component {
  ...
  renderOption(config, option, selected, onSelect, index) {
    ...
    let separatorStyle = [baseOptionContainerStyle, borderStyles]

    // Workaround to get seperators working on Android
    if (index === this.props.options.length - 2 && Platform.OS === 'android') {
      separatorStyle = [baseOptionContainerStyle, borderStyles, { borderRightWidth: 1, borderRightColor: config.separatorTint }]
    }
